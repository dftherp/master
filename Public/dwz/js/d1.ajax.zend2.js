 ...
 * 
 */
function navTabAjaxDone(json,form){
    DWZ.ajaxDone(json);
 ...
 * @param form
 */
function showValidatorErrors(json,form){
    if(json.statusCode == DWZ.statusCode.error){
        if(json.formMessage){
            var validateMsg;
            var errorMsg={};
            var validator = form.validate();
            $.each(json.formMessage,function(i,n){
                validateMsg=n;
                if($.isPlainObject(n)){
                    validateMsg=[];
                    $.each(n,function(type,content){
                        validateMsg.push(content);
                    });
                    validateMsg=validateMsg.join(" ");
                }
                errorMsg[i]=validateMsg;
            });
...
function dialogAjaxDone(json,form){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		}else if(json.dialogRel){
            var dialog=$("body").data(json.dialogRel);
            if(dialog){
                $.pdialog.reload(dialog.data('url'),{dialogId:json.dialogRel});
            }
        }
		if ("closeCurrent" == json.callbackType) {
