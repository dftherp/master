function initEnv() {
	...
	if ($.fn.switchEnv) $("#switchEnvBox").switchEnv(); //不需要
	if ($.fn.navMenu) $("#navMenu").navMenu();
		
	setTimeout(function(){
		initLayout();
		initUI();
        navTab.init(); //修改 在layout之后才能初始化
		// navTab styles
...
function initUI(_box){
    ...
    if ($.fn.d1mExport) $("a[target=d1mExport]", $p).d1mExport();
    if ($.fn.xheditor) {
        $("textarea.template-editor", $p).each(function(){
            var $this = $(this);
            var op = {html5Upload:false, internalScript:true,inlineScript:true,internalStyle:true,inlineStyle:true,
                sourceMode:true,width:$this.attr("width")||'500px',height:$this.attr("height")||"300px",skin: 'vista',tools: $this.attr("tools") || 'full'};
            var upAttrs = [
                ["upLinkUrl","upLinkExt","zip,rar,txt"],
                ["upImgUrl","upImgExt","jpg,jpeg,gif,png"],
                ["upFlashUrl","upFlashExt","swf"],
            ...
            var selectAttr=["urlBase","urlBase","modalWidth","modalHeight","modalTitle","upBtnText"];
            $(selectAttr).each(function(i,attr){
            ...
            var editor=$this.xheditor(op);
            $this.data('editor',editor);
        });
    }
    $("button[editorTarget]",$p).click(function(){
        var $this = $(this);
        var editor=$("#"+$this.attr('editorTarget')).data('editor');
        if(editor){
            editor.toggleSource(false);
            editor.pasteHTML($this.attr('html'),false);
