 ...
 */
(function($){
	var _lookup = {currentGroup:"",strDot:"" ,suffix:"", $target:null, pk:"id"};
	var _util = {
		_lookupPrefix: function(key){
			return _lookup.currentGroup + _lookup.strDot + key + _lookup.suffix;
		},
	...
	$.fn.extend({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
                        ...
                        fieldHiddens: $th.attr("fieldHiddens") || ""
					};
					fields.push(field);
				});
			...
			function tdHtml(field){
				...
				var suffixFrag = suffix ? 'strDot="'+strDot+'" suffix="' + suffix + '" ' : '';
                ...
                var readonly='';
                if(field.fieldClass.indexOf("readonly") >= 0){
				...
				switch(field.type){
                    ...
                    case 'lookup':
						...
						html = '<input type="hidden" name="'+field.lookupGroup+strDot+field.lookupPk+suffix+'"/>'
							+ '<input type="text" name="'+field.name+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" size="'+field.size+'" class="'+field.fieldClass+'"'+readonly+'/>'
							+ '<a class="btnLook" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suffixFrag+' lookupPk="'+field.lookupPk+'" title="查找带回">查找带回</a>';
						break;
					case 'attach':
						html = '<input type="hidden" name="'+field.lookupGroup+strDot+field.lookupPk+suffix+'"/>'
					...
					case 'enum':
						...
						break;
					case 'date':
						html = '<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" class="date '+field.fieldClass+'" dateFmt="'+field.patternDate+'" size="'+field.size+'"'+readonly+'/>'
							+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
						break;
					default:
						html = '<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" size="'+field.size+'" class="'+field.fieldClass+'" '+attrFrag+readonly+'/>';
						break;
				}
                if(field.fieldHiddens){
                    var fieldHiddens=field.fieldHiddens.split(",");
                    $.each(fieldHiddens,function(i,fieldHidden){
                        html+='<input type="hidden" name="'+field.lookupGroup+strDot+fieldHidden+suffix+'"/>';
                    });
                }
				return '<td>'+html+'</td>';
			}
